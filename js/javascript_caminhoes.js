(function readyJS(win,doc){
	...
	function ajaxFunction(action, brandId="", vehicleId="", yearId="")
	{
		...
		//url: getRoot()+"nfe-guard/controller/controllerFipeCaminhoes.php?action="+action+brandId+vehicleId+yearId+"",
		url: getRoot()+"controller/controllerFipeCaminhoes.php?action="+action+brandId+vehicleId+yearId+"",
		type: "POST",
		dataType: "JSON",
		async: false,
	...
	function getBrand()
	{
		...
		$("#brand").on("change", function(){
			///console.log($(this)val());
				...
				$('#vehicles').find('option').remove().end().append('<option value="">Selecione o caminhão</option>');
	...
	function getPreco(brandId, vehicleId, yearId)
	{
		...
		$.getJSON("https://fipeapi.appspot.com/api/1/caminhoes/veiculo/"+brandId+"/"+vehicleId+"/"+yearId+".json", function(data){
	...
	function getCalc(precoId, yearId)
	{
		...
		var a = numeros*0.01; //aceita numero ou string
