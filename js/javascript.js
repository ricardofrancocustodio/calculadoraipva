(function readyJS(win,doc){
	...
	function ajaxFunction(action, brandId="", vehicleId="", yearId="")
	{
		...
		//url: getRoot()+"nfe-guard/controller/controllerFipe.php?action="+action+brandId+vehicleId+yearId+"",
		url: getRoot()+"controller/controllerFipe.php?action="+action+brandId+vehicleId+yearId+"",
		type: "POST",
		dataType: "JSON",
		async: false,
	...
	function getBrand()
	{
		...
		$("#brand").on("change", function(){
				...
				$('#vehicles').find('option').remove().end().append('<option value="">Selecione o veiculo</option>');
	...
	function getPreco(brandId, vehicleId, yearId)
	{
		...
		$.getJSON("https://fipeapi.appspot.com/api/1/carros/veiculo/"+brandId+"/"+vehicleId+"/"+yearId+".json", function(data){
	...
	function getCalc(precoId, yearId)
	{
		...
		var a = numeros*0.03; //aceita numero ou string
